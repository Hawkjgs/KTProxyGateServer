...
public class Seed256 {
	...
	public static String Decrypt (byte[] hash) throws Exception {
...
//				result = AnsiX923Padding.removePadding (result, 0);
//				System.out.println("TT:"+new String (result, 0, result.length));
	...
	public static String DecryptShort(byte[] hash) throws Exception {
		byte[] result = new byte[hash.length];
		int pdwRoundKey[] = new int[KEY_SIZE];
		SeedRoundKey (pdwRoundKey, Key.getBytes ());
...
//				System.out.println("TT:"+new String (result, 0, result.length));
	...
	public static String DecryptShort(String hash) throws Exception{
		if(StringUtil.isEmpty (hash)) {
			return null;
		}
		if(hash.length () == 1) {
